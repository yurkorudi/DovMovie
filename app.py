...
EMAIL = 'lviv.dovzhenkocentre@gmail.com'
PASSWORD = 'ypgg dhnr aubi kzyi'
...
def send_dovzhenko_ticket_email(
    recipient,
    movie_title,
    session_datetime,
    pdf_bytes,
...
"""
    ...
    msg["Subject"] = f"🎬 Квиток на фільм «{movie_title}»"
    ...
    msg.set_content(
        f"""Дякуємо за покупку!
            ...
            Фільм: {movie_title}
            Сеанс: {session_datetime}

            Ваш квиток знаходиться у вкладенні.
            """
                )
...
def ticket_pdf():
    ...
    if sess and getattr(sess, 'dateTime', None):
        dt_str = (sess.dateTime + timedelta(hours=3)).strftime('%Y-%m-%d %H:%M') #hourChange
    else:
        dt_str = "-"
...
def admin_reports():
    ...
    for ticket, session, film in tickets:
        key = (film.title, (session.dateTime + timedelta(hours=3)).strftime('%d.%m %H:%M'), ticket.cost)
